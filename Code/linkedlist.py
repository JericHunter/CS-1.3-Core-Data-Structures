...
class LinkedList(object):
    ...
    def length(self):
        ...
        Best and worst case running time: O(1) because we keep track of size using other functions"""
    ...
    def get_at_index(self, index):
        ...
        Best case running time: O(1)if our index is at the head/tail
        Worst case running time: O(i) if we are inserting anywhere other than near hea/taild"""
    ...
    def insert_at_index(self, index, item):
        ...
        raise ValueError if the given index is out of range of the list size.
        Best case running time: O(1)if our index is at the head/tail
        Worst case running time: O(i) if we are inserting anywhere other than near head/tail"""
        # Check if the given index is out of range and if so raise an error
        if not (0 <= index <= self.size):
            raise ValueError('List index out of range: {}'.format(index))
    ...
    def append(self, item):
        ...
        Best and worst case running time: O(1) because we keep track of the tail"""
    ...
    def prepend(self, item):
        ...
        Best and worst case running time: O(1) because we keep track of the head [TODO]"""
        # Create a new node to hold the given item
        new_node = Node(item)
        # Check if this linked list is empty
        if self.is_empty():
    ...
    def find(self, quality):
        ...
        not present and we need to traverse through all n nodes in the list."""
        # Start at the head node
        node = self.head  # Constant time to assign a variable reference
        # Loop until the node is None, which is one node too far past the tail
  ...
  Best case running time: O(1) we are replacing the value near the head node.
        Worst case running time: O(n) We have to traverse through each node to find the item"""
